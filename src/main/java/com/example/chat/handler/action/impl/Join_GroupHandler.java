 ...
 * 加入群组处理器
 */
@Component("JOIN_GROUP")
public class Join_GroupHandler extends BaseActionHandler {
    ...
    public void handle(WebSocketSession session, WsRequest request) {
        try {
            ...
            // 调用 UserService 加入群组
            boolean success = userService.joinGroup(groupId, userId);
            ...
            sendError(session, "加入群组失败: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
