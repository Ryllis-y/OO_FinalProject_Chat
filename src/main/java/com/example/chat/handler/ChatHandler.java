...
public class ChatHandler extends TextWebSocketHandler {
    ...
    private void handleMessage(WebSocketSession session, WsRequest request) {
        ...
        try {
            ...
            // 2. 解析参数
            if (request.getParams() == null || !request.getParams().has("targetUser") || !request.getParams().has("content")) {
    ...
    private void handleCreateGroup(WebSocketSession session, WsRequest request) {
        // 1. 检查参数
        if (request.getParams() == null || !request.getParams().has("groupName")) {
    ...
    private void handleRecallMessage(WebSocketSession session, WsRequest request) {
        if (request.getParams() == null || !request.getParams().has("msgId")) {
        ...
        if (success) {
            // 2. 撤回成功，必须广播通知大家！
            // 构造一个通知包
            java.util.Map<String, Object> recallData = new java.util.HashMap<>();
            recallData.put("recalledMsgId", msgId);
            recallData.put("operatorName", operator);
    ...
    private void handleKickUser(WebSocketSession session, WsRequest request) {
        ...
        if (request.getParams() == null || !request.getParams().has("targetUser")) {
    ...
    private void handleGetHistory(WebSocketSession session, WsRequest request) {
        ...
        long beforeTime = (request.getParams() != null && request.getParams().has("beforeTime"))
    ...
    private void handleMuteUser(WebSocketSession session, WsRequest request) {
        ...
        if (request.getParams() == null || !request.getParams().has("targetUser") || !request.getParams().has("duration")) {
